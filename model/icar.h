...
struct PacketQuEntry
{
 	...
 	UnicastForwardCallback m_ucb;
  	...
  	PacketQuEntry(Ptr<Packet> p, Ipv4Header h, UnicastForwardCallback u)
 	{
 		this->m_packet = p;
 		this->m_header = h;
 		this->m_ucb = u;
...
struct DelayPacketQuEntry
{
    ...
    Ipv4Address m_nexthop;
  	...
  	DelayPacketQuEntry(Ptr<Packet> p, Ipv4Header h, UnicastForwardCallback u, Ipv4Address nexthop)
 	{
 		this->m_packet = p;
 		this->m_header = h;
 		this->m_ucb = u;
...
struct QPacketInfom
{
 	...
 	Ipv4Address dst;
  	...
  	QPacketInfom(Ipv4Address src, Ipv4Address dst)
 	{
 		this->src = src;
 		this->dst = dst;
 	}

  	QPacketInfom(const QPacketInfom& info)
 	{
 		*this = info;
 	}

  	bool operator< (const QPacketInfom& info) const
 	{
...
typedef std::map<QPacketInfom, Time> QqMap;

struct SendingQueueIcar
{
 	typedef Ipv4RoutingProtocol::UnicastForwardCallback UnicastForwardCallback;
 	...
 	Ipv4Address nexthop;
  	...
  	SendingQueueIcar(Ptr<Packet> p, Ipv4Header h, UnicastForwardCallback u, Ipv4Address nexthop)
 	{
 		this->m_packet = p;
 		this->m_header = h;
 		this->m_ucb = u;
...
private:
    ...
    icar::MessageList m_queuedMessages;
...
/*------------------------------------------------------------------------------------------*/
    ...
    std::vector<SendingQueueIcar> m_squeue;
    std::vector<PacketQuEntry> m_pqueue;		
    std::vector<PacketQuEntry> m_pwaitqueue;	
    std::vector<DelayPacketQuEntry> m_delayqueue;	
...
/*------------------------------------------------------------------------------------------*/
    ...
    //计算道路得分
    void RSE(const icar::MessageHeader &msg);
/*------------------------------------------------------------------------------------------*/
//设置协议所需的定时器
    //用以定时发送Beacon
...
/*------------------------------------------------------------------------------------------*/
    ...
    void ProcessCP (const icar::MessageHeader &msg, const Ipv4Address receiverIfaceAddr, const Ipv4Address senderIface);
    
    //Beacon机制处理逻辑
    void SendHello ();
    void ProcessHello (const icar::MessageHeader &msg, const Ipv4Address receiverIfaceAddr, const Ipv4Address senderIface);
    
    //Beacon机制的具体发送机制
    void SendPacket (Ptr<Packet> packet);
    void QueueMessage (const icar::MessageHeader &message, Time delay);
