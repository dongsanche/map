...
NS_LOG_COMPONENT_DEFINE ("icarHeader");
...
{
	static TypeId tid = TypeId("ns3::icar::BlockPacketHeader")
			.SetParent<Header>()
			.SetGroupName("icar")
...
{
  static TypeId tid = TypeId ("ns3::icar::CtrPacketHeader")
    .SetParent<Header> ()
    .SetGroupName ("icar")
...
{
  static TypeId tid = TypeId ("ns3::icar::DataPacketHeader")
    .SetParent<Header> ()
    .SetGroupName ("icar")
...
{
  static TypeId tid = TypeId ("ns3::icar::MessageHeader")
    .SetParent<Header> ()
    .SetGroupName ("icar")
...
MessageHeader::Hello::GetSerializedSize (void) const
{
    ...
    for(std::vector<JunInfom>::const_iterator itr = this->conlist.begin(); itr != this->conlist.end(); itr++)
    {
